...
# AAの箱「AとDは同種の箱　AとBの箱のどちらも奇数ではない　AAの箱は赤か青　　　実際は AとDは異種　AとBの箱は偶数ドルではない AAの箱は赤でも青でもない
ensuresA = [
    is_honest[AA] == is_honest[DD],
    fold_or(
...
ensuresB = [
    ...
    fold_and(
        ...
        ~(COLORandPRICE[RED, P5] & COLORandPRICE[BLUE, P1]),
    )
]

# CCの箱「AがCを虚偽の箱だといった　DとEの箱のどちらかが緑の箱である　Eの箱は4ドルである　真実
ensuresC = [
    then(is_honest[AA], ~is_honest[CC]),
...
# EEの箱「Eは緑の箱だ　Dの箱のほうがBより高い　Bは2ドルだ」　実際は Eは緑の箱ではない　Bの箱のほうがDの箱より高い値段である　Bは2ドルではない
ensuresE = [
    NAMEandCOLOR[EE, GREEN],
    fold_or(
