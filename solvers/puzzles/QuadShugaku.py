...
def solve_quad_shugaku(height, width, problem):
    problems = []
    for i in range(4):
    ...
    for i in range(4):
        sat, ans1, ans2 = Shugaku.solve_shugaku(height, width, problems[i])
...
def generate_quad_shugaku(height, width, verbose=True, symmetry=False):
    def penalty(problem):
        ret = 0
        for y in range(height):
            for x in range(width):
    ...
    generated = generate_problem(lambda problem: solve_quad_shugaku(height, width, problem),
...
def generatehxw(height, width):
    print("Generating")
    problem = generate_quad_shugaku(height, width, verbose=False)
    ...
    print(Shugaku.serialize_shugaku(final_problem))
