...
# puz_type/y_size/x_size/y_keys/x_keys/y0_clues/y1_clues/y2_clues/y3_clues.../yn_clues
...
s_y_keys = solver.int_array(height, -100, 100)
key_x = solver.int_array(width, -100, 100)

for y in range(height):
    if p_y_keys[y] != '-':
        solver.ensure(s_y_keys[y] == int(p_y_keys[y]))

for x in range(width):
    if p_x_keys[x] != '-':
        solver.ensure(key_x[x] == int(p_x_keys[x]))

s_problem = solver.int_array((height, width), -50, 50)
...
for y in range(height):
    solver.ensure(sum(is_white[y, x].cond(s_problem[y, x], 0) for x in range(width)) == s_y_keys[y])
...
if solver.solve():
    ...
    #                 for y in range(height)]
    is_white_str = [['? ' if is_white[y, x].sol is None else str(s_problem[y, x].sol) + ' ' if is_white[y, x].sol else '# ' for x in range(width)]
                    for y in range(height)]
